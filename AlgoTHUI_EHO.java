...
public class AlgoTHUI_EHO {
	...
	//file paths
	final String dataset = "fruithut";
	final String input = "D:\\install\\idea_soft\\spmf\\ca\\pfv\\spmf\\test\\mytest\\"+dataset+".txt";
	String output = "output_THUI_EHO.txt";
	//Algorithm parameters
	final int pop_size = 20; // the size of the population
	final int iterations = 1000; // the number of iterations before termination
	...
	// 定义了一个名为 Pair 的私有静态内部类，它实现了 Comparable<Pair> 接口
	private static class Pair implements Comparable<Pair> {
        ...
        //compareTo 方法：用于定义 Pair 对象的比较规则
		// 如果当前对象的 item 小于或等于另一个 Pair 对象的 item，则返回 -1，否则返回 1
	...
	private static class Item implements Comparable<Item> {
		...
		//将 Item 对象转换为字符串时，只返回项的名称（即 item 的值）
	...
	private static class Particle implements Comparable<Particle> {
		BitSet X; // itemset of particle (编码向量)
	...
	//定义了一个名为 Solutions 的私有内部类，用于维护前 k 个最优解
	private class Solutions {
		//表示集合能容纳的最大元素数量，即 k
		final int capacity;
		//sol是一个 TreeSet ，用于存储 Particle 对象，并按照 Comparator.reverseOrder() 进行排序，这意味着较大的值会排在前面。
		TreeSet<Particle> sol = new TreeSet<>(Comparator.reverseOrder());
		//reversed for faster Roulette wheel sel.
	...
	private void update() {
		for (int i = 0; i < pop_size; i++) {
			...
			List<Integer> diffList = bitDiff(gBest, p);//different items between gBest and current particle
	...
	public void printStats() {
		...
		System.out.println(" High-utility itemsets count : " + solutions.getSize());
		System.out.println("==================================");
	}
